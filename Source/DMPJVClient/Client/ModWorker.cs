...
namespace DarkMultiPlayer
{
    public class ModWorker
    {
        ...
        public bool ParseModFile(string modFileData)
        {
            ...
            //Err...
            string tempModFilePath = Path.Combine(Path.Combine(Path.Combine(Path.Combine(Path.Combine(KSPUtil.ApplicationRootPath, "GameData"), "DMPJointVentureClient"), "Plugins"), "Data"), "DMPModControl.txt");
            ...
            if (isWhiteList)
            {
                ...
                autoAllowed.Add("DMPJointVentureClient/plugins/darkmultiplayer.dll");
                autoAllowed.Add("DMPJointVentureClient/plugins/darkmultiplayer-common.dll");
                //Leave the old one there if the user forgets to delete it.
                autoAllowed.Add("DMPJointVentureClient/plugins/messagewriter.dll");
                autoAllowed.Add("DMPJointVentureClient/plugins/messagewriter2.dll");
                //Compression
                autoAllowed.Add("DMPJointVentureClient/plugins/icsharpcode.sharpziplib.dll");
        ...
        public void GenerateModControlFile(bool whitelistMode)
        {
            ...
            foreach (string modDirectory in modDirectories)
            {
                ...
                if (lowerDirectoryName.StartsWith("DMPJointVentureClient"))
                {
                    continue;
                }
