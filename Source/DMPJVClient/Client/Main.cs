...
namespace DarkMultiPlayer
{
    ...
    public class Client : MonoBehaviour
    {
        ...
        public void Awake()
        {
            ...
            UnityEngine.Debug.Log("DMP - Joint Venture 0.4 installed at " + assemblyPath);
        ...
        {
            string darkMultiPlayerSavesDirectory = Path.Combine(Path.Combine(KSPUtil.ApplicationRootPath, "saves"), "DMPJointVentureClient");
            ...
            string darkMultiPlayerCacheDirectory = Path.Combine(Path.Combine(Path.Combine(KSPUtil.ApplicationRootPath, "GameData"), "DMPJointVentureClient"), "Cache");
            CreateIfNeeded(darkMultiPlayerCacheDirectory);
            string darkMultiPlayerIncomingCacheDirectory = Path.Combine(Path.Combine(Path.Combine(Path.Combine(KSPUtil.ApplicationRootPath, "GameData"), "DMPJointVentureClient"), "Cache"), "Incoming");
            CreateIfNeeded(darkMultiPlayerIncomingCacheDirectory);
            string darkMultiPlayerFlagsDirectory = Path.Combine(Path.Combine(Path.Combine(KSPUtil.ApplicationRootPath, "GameData"), "DMPJointVentureClient"), "Flags");
            CreateIfNeeded(darkMultiPlayerFlagsDirectory);
        }

        private void SetupBlankGameIfNeeded()
        {
            string persistentFile = Path.Combine(Path.Combine(Path.Combine(KSPUtil.ApplicationRootPath, "saves"), "DMPJointVentureClient"), "persistent.sfs");
            if (!File.Exists(persistentFile))
            {
                DarkLog.Debug("Creating new blank persistent.sfs file");
                Game blankGame = CreateBlankGame();
                HighLogic.SaveFolder = "DMPJointVentureClient";
        ...
        private Game CreateBlankGame()
        {
            ...
            returnGame.Title = "DMPJointVentureClient";
